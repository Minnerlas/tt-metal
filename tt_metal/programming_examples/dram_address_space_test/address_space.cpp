...
int main() {
    try {
        ...
        const uint32_t num_l1_buffers = 1;  // 256 / 4;
        constexpr uint32_t num_iter = 1;
        constexpr uint32_t num_tiles = 1;
        constexpr uint32_t elements_per_tile = 1 << 15;
        ...
        // InterleavedBufferConfig dram2_config {
        ...
        ReplicatedBufferConfig dram_buffer_config2{
            // Size per device (replicated across mesh). Since we are
            // operating on a unit mesh this is the total size.
            .size = tile_size_bytes,
        };
        ...
        auto out1_buff = MeshBuffer::create(dram_buffer_config2, dram_config, mesh_device.get());
        auto out2_buff = MeshBuffer::create(dram_buffer_config2, dram_config, mesh_device.get());

        // auto input2_dram_buffer = MeshBuffer::create(dram_buffer_config, dram2_config, mesh_device.get());
        ...
        if (num_dram_channels > corelist.size()) {
            fmt::print(stderr, "Asked for {} cores, have only {}\n", num_dram_channels, corelist.size());
            exit(1);
        }
        ...
        TensorAccessorArgs(*out1_buff->get_backing_buffer()).append_to(dram_copy_compile_time_args);
        TensorAccessorArgs(*out2_buff->get_backing_buffer()).append_to(dram_copy_compile_time_args);
        ...
        KernelHandle dram_copy_kernel_id = CreateKernel(
            program,
            OVERRIDE_KERNEL_PREFIX "dram_address_space_test/kernels/address_space_kernel.cpp",
            cores,
            DataMovementConfig{
        ...
        std::vector<uint32_t> result_vec, vec1, vec2;
        ...
        // uint64_t bytes_read = 2 * num_dram_channels * num_iter * num_l1_buffers * num_tiles * tile_size_bytes;
